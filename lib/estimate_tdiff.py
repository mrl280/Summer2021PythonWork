...
def estimate_tdiff_helper(axes, df, tdiff_range_ns):
    ...
    num = int((extended_tdiff_range_ns[1] - extended_tdiff_range_ns[0] + 1) * 1.5)
    ...
    for tdiff in tdiffs_ns:
        ...
        df["elv_for_" + str(tdiff)] = df['adjElv']
    ...
    for t in range(len(tdiffs_ns) - 1):
        ...
        elv_init_key = "elv_for_" + str(tdiff_init)
        elv_final_key = "elv_for_" + str(tdiff_final)
...
if __name__ == "__main__":
    ...
    tdiff, fig = estimate_tdiff(df=df)
    ...
    out_fig = out_dir + "/estimate_tidff_" + station + year + month + day
